...
@pytest.mark.smoke
def test_crawl_app_reanchors_before_fallback_tap_after_failed_memory_navigation():
    class PageIdPolicy:
        def candidates(self, _scene):
            return [
                {
    ...
    class PartialMemoryPathApp(_FakeApp):
        def __init__(self):
            super().__init__(
                {
                    "home": [("设置", "settings")],
    ...
    assert report.navigations == 1
    assert app.memory_targets == ["app/settings"]
    assert app.taps == [_box_for(0).center]
    assert app._cur == "settings"
    assert [(entry.label, entry.source, entry.outcome) for entry in report.entries] == [
        ("设置", "unit_policy", "navigated")
    ]
