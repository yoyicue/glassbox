...
> 仍未完整建立，但已有单 App multi-sample outcome snapshot：iPad mini 7 Settings n=5（code
> `d9695ae`，artifact `/tmp/glassbox-l2-rank2-full-20260606-164702/benchmark.json`，命令
> `GLASSBOX_PHONE_MODEL=ipad_mini_7 ... run-ios-settings --rounds 5 --drill-down --language en --region HK`）
> 跑出 4/5 succeeded、`task_completion_rate=0.8`、`task_completion_variance=0.16`、
> `expected_state_coverage=0.976`、`recoveries=0`，失败样本缺 `隐私与安全性`；人类对照协议/模板/校验器
> 已落地，但仍缺实际人类 trial、多 App/语言扩展和 floor 冻结。去敏 snapshot 已提交为
> `skills/regression/fixtures/l2_settings_expected_state_snapshot.json`，并有 smoke 验证其 schema、
> n≥5、coverage 非零和 coverage-regression gate；人类模板为
> `skills/regression/fixtures/human_baseline_settings_template.json`，由
> `skills.regression.human_baseline` 校验；`validate-floor-candidate` 已将当前 4/5 snapshot 明确拒绝为
> floor 候选。第②层正在修正但地板仍不诚实（2026-06-06
> `tap_element` 已接入 semantic `tap` ladder，Settings row/search root result tap 已带 action-level
> `page_id` expected-state，且 verifier 支持 `page_id.any_of` 以覆盖中英/HK alias；但 committed baseline
> 仍是 2026-06-01 的 5/5 completion floor，coverage-bearing snapshot 尚未升格为 completion floor）。普通
> HDMI Settings floor 不应混入 Voice Control item-number/name overlay、FKA/help overlay 或
> Stage Manager/windowed Settings 状态；`validate-floor-candidate` 已要求 clean HDMI cell/environment，
> 并会拒绝明显数字 overlay 标记的 final-state。这些 a11y/windowed 状态会改变 observation distribution，
> 应作为单独 a11y-mode eval cell 报告，而不是默认 floor。第③层部分修正（2026-06-05：`compare_benchmarks` 已 gate
> coverage/process 回归和有样本时的 scroll 回归）。详见
> `docs/goals/computer_use_honest_gate_first.md`。
>
> **更新（2026-06-10，main `b61f079`）**：第②层已诚实化——committed floor 替换为 expected-state-verified
> 的 n=5 成功跑（`git_sha=dd74fbb`：completion/action/root 全 1.0、`expected_state_coverage=0.978`、
> `recoveries=2`），经 `validate-floor-candidate` 通过后人工升格。第③层补齐阻断牙：nightly 新增
> machinery probe（故障注入后策略阶梯+恢复必须触发，否则 rc1）——干净地板上 `vlm_action_coverage`/
> `strategy_switches`=0 是诚实零，"覆盖率不许跌"对救场类指标是反的（路径更可靠⇒救场更少），故以
> "注入故障后机器必须触发"替代。第①层余项 = 人类对照 trial 采集 + 多 cell（第二 App / zh）+
> verifier F1 人工对齐。
...
1. iOS/iPadOS out-of-band 真机评测是**领域空白**——若想严肃评测，须**自建 execution-based outcome
   ...
   （glassbox 的 UTG + expected-state 机制本可承担此角色。）Voice Control overlay 可能提供屏上
   label/value 线索，但它不是外部 a11y 树，也会改变 HDMI 图像；应单独建 a11y-mode eval cell。
...
4. 人类对照已有码内协议/模板/校验器；仍需采集匿名 human-control trials，并决定 human-control 数据只作为报告列，
   还是进入未来 floor 冻结/接受标准。
