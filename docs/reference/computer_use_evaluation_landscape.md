...
> **本仓现状的对照点**（代码事实，截至 `5881a28`，行号会漂移，引用时请回查源）：第①层缺失
> （无真正的 multi-sample outcome 评测）；第②层不诚实（committed 地板由绕过 orchestrator 的
> `tap_xy` 爬虫产生，`navigation.py:185/276/522`，故覆盖率指标 `expected_state_coverage /
> vlm_action_coverage / strategy_switches / recoveries / scroll_success_rate` 全 0）；第③层
> 错位（`compare_benchmarks` 在 `skills/regression/computer_use_success_rate.py:1223-1254` 只 gate
> 19 个里的 5 个，覆盖率指标只打印不 gate）。详见 `docs/goals/computer_use_honest_gate_first.md`。
