...
- **接线**：
  ...
  - **承重改动**：Settings root 行入口必须给每个 action 标注目标 `page_id` expected-state；2026-06-06
    第一片已落地：正常 row tap 走 `tap_element(..., expected_state={"kind": "page_id", ...})`，并支持
    中英/HK alias 的 `page_id.any_of`。剩余工作是把仍保留的坐标 fallback/search taps 收敛到同一语义出口，
    并按任务定义设置 `terminal_expected_state`。
...
- **现状**：🟡 **第一片已落地，L2 仍未完成**。正常 Settings row tap 已带 action-level
  expected-state，下一次真机 benchmark 应该让 `expected_state_coverage` 从 0 长出来；但还没有 n≥5
  多采样、人类对照、完整 semantic strategy ladder，committed floor 仍不是诚实 L2 outcome floor。
...
| **Rank 2** | 🟡 第一片：Settings row tap 标注 `page_id` expected-state；剩余：完整 semantic 出口、每任务 `terminal_expected_state`、n≥5 多采样 | L2 | 高·真机 | honest-gate；`_task_outcome:622` |
...
**关键依赖**：Rank 1 已给 Rank 2 的 coverage 指标"长出牙齿"；Rank 2 第一片已把正常 Settings row
tap 接上 action-level expected-state，但还需要真机 n≥5 证明 coverage 非零并产出 execution-based outcome；
Rank 3 才有诚实地板可冻结。L1/旁路排在 L2 跑通之后。
...
4. **人类对照基线**：谁、用什么协议跑同任务的人工完成率（L2 验收要求）。
