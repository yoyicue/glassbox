...
- **现状**：🟡 **承重代码已落地，单 App n≥5 已跑出 outcome；L2 验收仍未完成**。
  ...
  n≥5、`expected_state_coverage>0` 和 coverage-regression gate 行为。还没有人类对照；现有
  `reliability_baseline.json` 仍保留 2026-06-01 的 5/5 completion floor，尚未把这条 4/5
...
| **Rank 2** | 🟡 承重代码：`tap_element` semantic ladder + Settings row/search-result `page_id` expected-state；iPad Settings n=5 snapshot 已完成并提交去敏 fixture（4/5 succeeded，`expected_state_coverage=0.976`）；剩余：人类对照 + 是否升格为 completion floor/nightly floor | L2 | 高·真机 | honest-gate；`_task_outcome:622` |
...
下一步要补人类对照，并决定是否用这条 4/5 outcome 替换/并入现有 5/5 completion floor；Rank 3 才能把
最终 floor 变成 nightly 门禁。L1/旁路排在 L2 跑通之后。
