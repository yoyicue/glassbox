...
- **接线**：
  ...
  - **承重改动**：把 Settings 行入口的三处 `phone.tap_xy`（`navigation.py:185/276/522`）改走
    `phone.semantic` / `default_semantic_action_plan` 并标注每步 `terminal_expected_state`——这是
    让 outcome 真正 execution-based、让 coverage 非零的唯一办法（见 honest-gate 文档：是架构改动
    不是 flag flip）。
...
- **现状**：❌ **缺失**。现有 `run_full` 默认走 `tap_xy`，outcome 回退到 ACK、coverage=0、n=1。
  指标机器（§4）其实已就位，差的是**让任务走对路径 + 多采样 + 人类对照**。
