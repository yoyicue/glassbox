...
每层给出：**问题 / 指标 / 环境·频率 / 验收 / 接线 / 现状**。接线以 Makefile target / module path
为准；行号会漂移，引用前回查源。
...
- **接线（现有）**：
  ...
  - schema：`compare_benchmarks` 入口先跑 `validate_benchmark`（`computer_use_success_rate.py:1213`）；
    `regression-gate` 跑 completion floor `validate`、human-control template `validate`、以及门禁
    smoke。
...
- **现状**：🟡 **承重代码已落地，单 App n≥5 已跑出 outcome；L2 验收仍未完成**。
  ...
  现有 `reliability_baseline.json` 仍保留 2026-06-01 的 5/5 completion floor；`validate-floor-candidate`
  已把 floor 升格策略机器化，并会拒绝这条 4/5 coverage-bearing snapshot。普通 HDMI Settings floor
  与 Voice Control overlay 探索必须分 cell：floor 候选要求干净的全屏 Settings，Voice Control
  continuous overlay 关闭，FKA/help overlay 不在屏上，且没有 Stage Manager/windowed Settings。带
  item-number/name overlay 的 run 只算 a11y-mode 诊断样本；只有 overlay parser、badge-to-target
  mapping 和 reset recipe 通过自己的 labeled replay/真机验收后，才能作为单独 a11y eval cell 报告，
  不得混入默认 Settings floor。
...
- **机制**：
  ...
  - **真机门**（nightly，阻塞性）：`make regression-compare CANDIDATE=...` 拿新鲜
...
- **接线（现有）**：`make check: lint test regression-gate golden-audit`；
...
| **Rank 2** | 🟡 承重代码：`tap_element` semantic ladder + Settings row/search-result `page_id` expected-state；iPad Settings n=5 snapshot 已完成并提交去敏 fixture（4/5 succeeded，`expected_state_coverage=0.976`）；floor-candidate policy 已落地并拒绝当前 4/5 snapshot；人类对照协议/模板/校验器已落地但 trial 数据待采集；剩余：清理普通 floor 验收环境（Voice Control/FKA/Stage Manager off、全屏 Settings）+ 真实人类 trial + 通过 floor policy 的新候选进入 nightly floor。Voice Control overlay 另走 a11y eval cell | L2 | 高·真机 | honest-gate；`_task_outcome:622` |
...
人类对照的协议/模板/校验器现在也有离线 smoke 保护；floor-candidate policy 已把 4/5 snapshot 排除在
completion floor 之外。下一步要先固定普通 Settings floor 的设备状态边界（无 Voice Control overlay、
无 FKA/help、无 Stage Manager 小窗口），再采集真实人类 trial，并跑出通过 floor policy 的新候选；
Rank 3 才能把最终 floor 变成 nightly 门禁。Voice Control overlay 的价值是给 verifier 提供更强的
屏上 label/value 线索，但它改变观测分布，必须单独报告。
L1/旁路排在 L2 跑通之后。
...
- **L2**：被测任务每个关键 action 带 expected-state、任务带 terminal_expected_state、coverage 非零、n≥5
  出 `task_completion_rate`+方差+人类对照；普通 Settings floor 的设备状态干净且全屏，Voice Control
  overlay 只在单独 a11y cell 验收。
...
1. **L2 的 outcome verifier**：iOS/iPadOS out-of-band 无 a11y 树/系统状态访问，仅凭屏幕观测如何
   ...
   `docs/design/glassbox_a11y_channels.md`（rank-1 Voice Control overlay）。注意：这不是默认
   HDMI floor 的验收通道；Voice Control overlay 必须作为显式 a11y-mode eval cell 单列。
