...
### 5.2 Post-floor residual fixes(操作按钮 carousel + 空搜索框假阳)

诊断基线:`iphone_floor_n5_post99`(honest floor `git_sha` baa6274,5 轮,
en/HK)。该 floor 的 `task_completion_rate=0.0` 由唯一残留决定:**必需根行
操作按钮(ACTION_BUTTON)5/5 轮都没进**。后取证(post-fix forensics)证明
两处修复**联合充分**把它从 0.0 → 1.0。两处都按本仓约定"修核心不修技能 /
veto+anchor+abstain"落地;离线 `make check` 可证(rig 复测前 floor 夹具不动,
其 0.0 仍诚实,floor-lineage 守卫照旧)。

1. **操作按钮 carousel 页身份(core,`glassbox/ios/scene.py`)。** iOS 26 的
   操作按钮"选什么动作"页是个近乎无字的 *轮播*:整页正文只有一张居中靠下
   (cy≈0.62h)的大字幕(记录里只见到首张 "Silent Mode")+ 一两行说明文
   ("Switch between Silent and Ring for calls / and alerts.")+ 右上一个可选
   的 "+" 加号,**无导航栏标题、无明细行**。`settings_detail_semantic_guess`
   只看到字幕 → 铸 `settings/Silent Mode`,被该行 expected-state 的 any_of
   (`settings/操作按钮` / `settings/Action Button` / `com.apple.settings.action-button`)
   拒,爬虫判"未进入"。修复:`action_button_carousel_guess` 在语义猜测之前
   命中——anchor 用**已知字幕族**(可指派动作:Silent Mode / Focus / Camera /
   Torch / Voice Memo / Magnifier / Translate / Shortcut / Accessibility / …)
   + 居中靠下字幕 + 紧随其下的说明行;veto 用**稀疏页**(≤6 非状态栏文本)+
   无导航栏标题 + 无 back + 无明细行;abstain 否则原样落到既有级联。铸
   `settings/Action Button` → 该行 verify `succeeded`。守卫(a):真实 声音与触感
   页(正文有一行 "Silent Mode",但有导航栏标题 + 十几行明细)被稀疏/无明细
   veto 排除,**S3 的 `settings/Silent Mode → settings/Sounds & Haptics`
   re-mint 不变**(mint-flip 允许表零新增,语料/golden 字节稳定除应得翻转)。
   离线钉:`view_0012.ocr.json`(轮播记录形状)的 verifier-golden 现期望
   `Action Button`/`succeeded`;另加真实 声音与触感 不被重判的反向钉。

2. **底部搜索框空字段假阳(skill,`core.py` + `policy.py`)。** iPhone 设置
   搜索栏在底部;聚焦但为空时,软键盘 AutoFill 栏与"建议"列表会渗入底部带。
   `_is_bottom_search_query_candidate`(blanket cy≥850)把 "AutoFill" 与渗到
   cy≥850 的建议行(连 OCR 糊成 "Solnds & Hantieg" 的)读成查询文本 →
   `_clear_settings_search` 以为空字段仍有查询 → `open_root_label_via_search`
   返回 False、记 `search_no_result`,**全程一次 type 都没打**(5 轮 0 次
   type)。三段修复:(1) `_clear_settings_search` 对底部空字段(仅占位符、
   无 "×" 清除键)**短路 True**,跳过那段在空字段上 verify"unknown — 无进展"
   并杀掉 rung 的 Cmd+A/Backspace;(2) bottom-field 查询检测加键盘-chrome
   排除集(对齐 iPad top-field 的 Paste/AutoFill/Select/…)并把候选带**收到
   字段行**(cy ≈ field.y ± 28,而非 blanket ≥850),无 "×" 键时再用 canonical
   根标签 veto 渗入的建议行;(3) 诚实归因:rung 失败原因拆成
   `search_clear_failed` / `search_query_not_typed`(经 `settings_context`
   透传)vs 真正打了字的 `search_no_result`,三者都进
   `EXPECTED_NAVIGATION_FAILURE_REASONS` 与 verify_report 的"未进入即不要求
   出现在已访问页"白名单。离线钉:三张记录形状(空面板 / 键盘+AutoFill /
   建议渗入)喂 `settings_search_has_query_text` 断言 False;真实查询+"×"清除
   键的正向控制断言 True;mock-Phone 端到端:空字段下 rung 真打字 + 点结果行
   + 原因记 `search_no_result`。

夹具/隐私同 §5.1:全部按记录形状构造(轮播/键盘/建议文案均为通用 iOS UI
串,`find_personal_texts` 验证为空),未提交原始 OCR。
